...
// (doc.image no soporta webp ni gif). Aceptar otros formatos aqui produciria un fondo que
...
export class SubirFondoEventoUseCase {
  ...
  async execute(archivo: { buffer: Buffer; mimetype: string } | undefined): Promise<string> {
    ...
    if (!extension) {
      throw new BadRequestException('La imagen de fondo debe ser JPG o PNG');
    }

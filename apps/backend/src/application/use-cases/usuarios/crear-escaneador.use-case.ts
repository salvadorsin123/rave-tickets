...
import { CrearEscaneadorDto } from '@application/dtos/usuarios.dto';
import { UsuarioEntity } from '@domain/entities/usuario.entity';
import { RolNombre } from '@domain/enums/rol.enum';

@Injectable()
export class CrearEscaneadorUseCase {
  constructor(
    @Inject(USUARIO_REPOSITORY) private readonly usuarioRepository: UsuarioRepositoryPort,
    @Inject(PASSWORD_HASHER) private readonly passwordHasher: PasswordHasherPort,
  ) {}

  async execute(dto: CrearEscaneadorDto): Promise<UsuarioEntity> {
    ...
    return this.usuarioRepository.create({
      ...
      email: dto.email,
      passwordHash,
      rolNombre: RolNombre.ESCANEADOR,
    });
  }
}
